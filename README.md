...
4. “类型”基础选项为 `BootLoader`、`App`、`Parameter`，接口参数和配置文件使用相同值；
...
查询到固件后，界面按钮显示为“烧录”，并显示文件名、MD5、自动换算后的文件大小、版本和最终烧录地址。选择 `BootLoader` 时地址固定为 `0x08000000`；选择 `App` 或 `Parameter` 时地址取零部件接口的 `burn_addr`。
...
6. 校验通过后调用 ST-Link 烧录流程：`BootLoader` 使用 `0x08000000`，`App` 和 `Parameter` 使用零部件的 `burn_addr`。
...
用户在“确认烧录”对话框点击“是”后，“固件选择”窗口保持打开。程序另行显示“烧录过程”窗口，并实时输出固件下载、MD5和大小校验、ST-Link连接、擦除、写入、校验和完成状态。

烧录执行期间不能关闭过程窗口或固件选择窗口。烧录成功或失败后过程窗口仍然保留，“关闭”按钮此时才会启用；用户查看并确认日志后手动关闭过程窗口，然后可返回固件选择窗口继续操作或手动关闭软件。

### 航空器编号与EO单号筛选

“航空器编号”和“EO单号”由登录角色权限中的JSON布尔字段 `api_limit.after_sales` 控制：

- 严格为 `true`：界面显示两项，终端流程也会询问；
- 为 `false`、字段缺失或不是布尔值：界面隐藏两项，终端不询问，也不向版本查询接口传参。
...
输入内容会去除首尾空格，仅非空值会加入 `/openapi/versions/latest` 查询参数。两项内容可写入主程序目录的非敏感配置并在下次有售后权限时恢复；无售后权限时不会恢复旧值，并在后续保存选择时清空旧值。

### 烧录失败后在过程窗口重试

“烧录过程”窗口只显示简洁的业务错误，不向用户显示Python调用栈。例如未连接设备时仅显示：

```text
[✗] 烧录失败: 未找到 ST-Link 设备
```

烧录失败后窗口保持打开并启用“烧录”按钮。用户连接ST-Link后可直接点击该按钮重试；已经下载且通过MD5和大小校验的固件会被复用，不重复下载。烧录成功后“烧录”按钮禁用，避免误重复烧录。

### 固件状态选择

“固件选择”页面的“状态”为只读下拉框，基础选项为：

- `研发验证`（接口值 `0`）
- `局方批准`（接口值 `1`）
- `生产测试`（接口值 `2`）

“类型”和“状态”会根据登录响应中的角色 `api_limit.program`、`api_limit.status` 过滤，只显示当前用户有权使用的选项；限制数组为空表示该维度不限制。查询接口仍传真实类型值和状态整数，配置文件中的 `status` 保存为整数。若保存的旧选项不在当前权限范围内，界面自动选择第一个有权限的选项。


### 图形界面全片擦除

“固件选择”页面将红色“全片擦除”按钮放在“查询固件”按钮右侧，两者同一行并排显示。该操作与命令行 `-e/--erase` 使用相同执行路径：连接所选ST-Link、识别芯片并调用 `mass_erase()`。

全片擦除会永久删除目标芯片中的全部Flash数据。程序在执行前显示一次警告确认；取消时不会连接设备或执行擦除。确认后在独立过程窗口显示ST-Link连接和擦除日志。若擦除失败，窗口会在“关闭”按钮左侧启用红色“全片擦除”按钮，用户连接好设备后可在当前窗口重试；每次重试都会重新创建连接并在结束后释放ST-Link句柄。擦除成功后重试按钮保持禁用。

### 固件烧录记录

服务器固件每次实际执行ST-Link烧录后，程序都会提交一次烧录记录：

```text
POST /openapi/burn-records?token=<api_token>
Content-Type: application/json
```
...
`user_id`来自本次登录接口返回的`id`，`version_id`来自最新版本接口返回的`id`，`success`为本次烧录尝试是否成功。GUI失败后在同一窗口重试时，每次尝试分别提交记录。记录提交失败只显示警告，不会改变芯片实际烧录结果。全片擦除、本地固件、读取等没有服务器版本ID的操作不提交烧录记录。
